...
public class EconomieManager {
    ...
    private static final Logger logger =
            LoggerUtility.getLogger(EconomieManager.class, "html");
    ...
    public static void updateEconomie(Economie economie, int nbHabitants, Evenement evenement) {
        logger.debug("Début de la mise à jour de l'économie pour " + nbHabitants + " habitants" +
                (evenement != null ? " avec événement: " + evenement.getType() : ""));
        ...
        economie.setGainsTotal(economie.getGainsTotal()   + gains);
        economie.setPertesTotal(economie.getPertesTotal() + pertes);

        logger.debug("Nouvelle valeur de stycas : " + economie.getStycas());
        logger.debug("Fin de la mise à jour de l'économie");
    }
    ...
    private static void updateRessourcesImportees(int nbHabitants, Economie economie, Evenement evenement) {
        logger.debug("Mise à jour des ressources importées...");
        ...
        for (String nomRessource : ressourceRepository.getAllNomRessources()) {
            if (!economie.contientRessourceEnRessources(nomRessource)) {
                ...
                logger.debug("Ressource importée créée : " + nomRessource + ", quantité initiale: " + quantite);
            } else {
                ...
                logger.debug("Ressource importée " + nomRessource + " variation: " + variation + ", nouvelle quantité: " + nouvelleQuantite);

                res.setQuantite(nouvelleQuantite);
            }
        }

        logger.debug("Mise à jour des ressources importées terminée.");
    }
    ...
    private static void updateExportations(int nbHabitants, Economie economie, Evenement evenement) {
        logger.debug("Mise à jour des exportations...");
        ...
        for (Produit produit : economie.getProductions().values()) {
            if (!economie.contientProduitEnExportation(produit.getNom())) {
                ...
                logger.debug("Produit créé en exportation : " + produit.getNom() + ", quantité initiale: " + quantite);
            } else {
                ...
                logger.debug("Produit export " + produit.getNom() + " variation: " + variation + ", nouvelle quantité: " + nouvelleQuantite);

                exp.setQuantite(nouvelleQuantite);
            }
        }

        logger.debug("Mise à jour des exportations terminée.");
    }
    ...
    private static void updateProductions(int nbHabitants, Economie economie, Evenement evenement) {
        logger.debug("Mise à jour des productions...");
        ...
        for (int i = 0; i < nbProduits; i++) {
            ...
            if (!economie.contientProduitEnProduction(nomProduit)) {
                ...
                logger.debug("Produit créé en production : " + nomProduit + ", quantité initiale: " + quantite);
            } else {
                ...
                logger.debug("Produit " + nomProduit + " variation: " + variation + ", nouvelle quantité: " + nouvelleQuantite);

                prod.setQuantite(nouvelleQuantite);
            }
        }

        logger.debug("Mise à jour des productions terminée.");
    }
