...
public class Bouton extends AbstractFigure {
	...
	private void setCouleurBouton(Graphics g) {		
		...
		DessinStrategy.definirCouleur((Graphics2D) g, getCouleur());
	}
	...
	private void dessinerBouton(Graphics2D g) {
		setCouleurBouton(g);
        DessinStrategy.remplirForme(g, getForme());
        
        DessinStrategy.definirCouleur(g, Color.BLACK); // contour noir
        DessinStrategy.dessinerForme(g, getForme());
        ...
        DessinStrategy.afficherTexte(g, SimConfig.POLICE_BOUTON, getTexte(),positionTexte);
	}
