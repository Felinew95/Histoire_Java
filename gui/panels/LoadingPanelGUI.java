...
import gui.management.DessinStrategy;
...
public class LoadingPanelGUI extends JPanel {
	...
	protected void paintComponent(Graphics g1) {
		...
		DessinStrategy.definirCouleur(g,Color.WHITE);
		DessinStrategy.afficherTexte(g,new Font(Font.DIALOG,Font.BOLD,20),bar.getTexte()+"%",new Point((int) ((RoundRectangle2D)bar.getForme()).getCenterX(),(int)((RoundRectangle2D)bar.getForme()).getCenterY()+100));
		...
		DessinStrategy.afficherTexte(g,new Font(Font.DIALOG,Font.ITALIC,75), texte,new Point((int) ((int)bar.getForme().getBounds2D().getCenterX()),(int) ((int)bar.getForme().getBounds2D().getCenterY()-140 ) ));
	...
	private void afficherBar(Graphics2D g) {
	    ...
	    DessinStrategy.definirCouleur(g, bar.getCouleur());

	    int remplissageBar = (int) (pourcentage * largeurBarre / 100.0);   
	    DessinStrategy.remplirRoundRectangle(g,new Point((int) (x), (int) y),remplissageBar,taille_defaut_y,  100,100);
	}
